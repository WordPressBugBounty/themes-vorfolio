...
class Plugin_Notice {
	...
	private function load_hooks() {
		...
		add_action( 'wp_ajax_vorfolio_dismiss_notice', array( $this, 'dismiss_notice' ) );
	}
	...
	public function notice_install_plugin() {
		...
		if ( get_option( 'vorfolio_dismiss_notice' ) ) {
			return;
		}
		...
		$plugins_required    = array(
            array(
					'slug'       		=> 'gutenverse',
					'title'      		=> esc_html__( 'Gutenverse', 'vorfolio' ),
					'short_desc' 		=> esc_html__( 'Gutenverse is a WordPress blocks plugin, page builder, and website builder for the native Block Editor and Site Editor. Create fast, responsive websites without code using 57 blocks, 600+ templates, global styles, responsive controls, popup builder tools, and block theme support.', 'vorfolio' ),
  ...
  '1x' => 'https://ps.w.org/gutenverse-companion/assets/icon-128x128.png?rev=3162415',
				...
				array(
					...
					'title'      		=> esc_html__( 'Gutenverse Form', 'vorfolio' ),
					'short_desc' 		=> esc_html__( 'Gutenverse Form is a WordPress form plugin, contact form builder, block form plugin, and booking form builder for the native Block Editor. Create contact forms, booking forms, reservation forms, subscribe forms, lead forms, feedback forms, and custom WordPress forms without code.', 'vorfolio' ),
  ...
  '1x' => 'https://ps.w.org/gutenverse-form/assets/icon-128x128.png?rev=3135966',
		...
		wp_register_style( 'vorfolio-theme-notice', false );
		wp_enqueue_style( 'vorfolio-theme-notice' );
		ob_start();
		?>
		
				.vorfolio-simple-notice {
				
					padding: 24px !important; 
				
					border-left: 4px solid #007cba !important;
				}
				.vorfolio-simple-notice p:first-child {
				
					margin-bottom: 20px; 
					margin-top: 0;
				}
				.vorfolio-simple-notice p:last-of-type {
				
					margin-top: 0; 
					margin-bottom: 0;
				}
				.vorfolio-simple-notice .vorfolio-notice-title {
					font-size: 17px;
					font-weight: 600; 
					display: block;
					margin-bottom: 6px;
				}
				.vorfolio-simple-notice .vorfolio-notice-description {
					font-size: 13px;
					display: block;
				...
				.vorfolio-simple-notice .button-primary {
					padding: 4px 16px !important;
				}

			
				.vorfolio-simple-notice .notice-dismiss {
					background: none !important;
					box-shadow: none !important;
					opacity: 1;
				}
				
				.vorfolio-simple-notice .notice-dismiss:before {
					color: #a7aaad;
					padding: 0; 
				}
				
				.vorfolio-simple-notice .notice-dismiss:hover:before,
				.vorfolio-simple-notice .notice-dismiss:focus:before {
					color: #c90000;
				}
		...
		wp_add_inline_style( 'vorfolio-theme-notice', $custom_style );
		wp_register_script(
			'vorfolio-theme-notice',
			'',
			array( 'wp-api-fetch' ),
			'1.0.0',
			true
		...
		wp_enqueue_script( 'vorfolio-theme-notice' );
		...
		function sequenceInstall(plugin, pluginsInstalled) {
			return new Promise((resolve, reject) => {
				...
				const needUpdate = plugin.installed
					? VORFOLIOPluginUtils.isVersionGreater(plugin.req_version, pluginsInstalled[`${path}.php`].Version)
					: false;
		...
		document.addEventListener('DOMContentLoaded', () => {
			const notice = document.querySelector('.notice.is-dismissible.vorfolio-simple-notice');
				...
				setTimeout(() => {
					...
					if (dismissBtn) {
						dismissBtn.addEventListener('click', (e) => {
							e.preventDefault();
							jQuery.post(ajaxurl, {
								action: 'vorfolio_dismiss_notice',
								_ajax_nonce: nonce
							});
						});
					}
			...
			button.addEventListener('click', function (e) {
				...
				button.innerHTML = `<?php esc_html_e( 'Installing...', 'vorfolio' ); ?>`;
				...
				if (!hasFinishClass) {
					...
					sequence.then(() => {
						...
						button.innerHTML = `<?php esc_html_e( 'Install Failed, Try Again', 'vorfolio' ); ?>`;
		...
		wp_add_inline_script( 'vorfolio-theme-notice', $custom_script );

		?>
		<div class="notice notice-info is-dismissible vorfolio-simple-notice" data-nonce="<?php echo esc_attr( wp_create_nonce( "vorfolio_dismiss" ) ); ?>">
				<p>
					<strong class="vorfolio-notice-title"><?php esc_html_e( "Thank you For Installing Vorfolio Theme", "vorfolio" ); ?></strong>
					<span class="vorfolio-notice-description">
						<?php esc_html_e( "Unlock the full potential of your website with the recommended plugins.", "vorfolio" ); ?>
						<br/>
						<?php esc_html_e( "Activate it to explore exclusive extensions, ready-to-use demo templates, and powerful features that make building your site easier and more enjoyable.", "vorfolio" ); ?>
					</span>
				</p>
				<p>
					<a href="#" class="button button-primary" id="gutenverse-install-plugin"><?php echo esc_html__( "Install Recommended Plugins", "vorfolio" ); ?></a>
				...
				<p class="installing-warning" style="font-size: 12px; color: #666; display: none; margin-top: 5px;">
					<i><?php esc_html_e( "Don\'t refresh the page when installing recommended plugins", "vorfolio" ); ?></i>
				</p>
			</div>
		<?php
	}
	...
	public function dismiss_notice() {
		check_ajax_referer( 'vorfolio_dismiss' );

		if ( ! get_option( 'vorfolio_dismiss_notice' ) ) {
			update_option( 'vorfolio_dismiss_notice', true );
		}
